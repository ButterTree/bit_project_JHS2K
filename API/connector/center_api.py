...
def home():
    return render_template('index.html')  # 'index.html'의 내용을 참조하여 Page를 구성해 출력한다.
...
def let_me_shine():
    ...
    if request.method == 'POST':   # POST 방식으로 Request가 전달된 경우에의 동작이다.
        ...
        file_name = r'../image_2_style_gan/img/{}'.format(f.filename)  # 첨부한 Image가 업로드한 파일명과 형식 그대로 일단 저장될 위치를 지정한다.
        client_img_name = '../image_2_style_gan/img/{}_{}.png'.format(client_ip, time_flag)  # 첨부한 Image가 png 형식으로 다시 저장될 경로와 이름을 지정한다.
