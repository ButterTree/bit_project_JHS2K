    ...
    try:
        for x, y, w, h in faces:                       # 얼굴 x좌표, y좌표, 가로길이, 세로길이
            ...
            eyes = eye_cascade.detectMultiScale(
                face_gray, scaleFactor=2.3, minSize=(85, 85), maxSize=(95, 95))
            # 눈만 잘 찾도록 값 조정
        ...
        # 눈의 좌표가 얼굴 내부 기준이므로 얼굴좌표를 더해줌
        eyes_loc = eyes + [[x, y, 0, 0]]

        # 눈이 있을 높이 내에서만 (입 인식 방지), 마스킹 잘 되도록 적절한 위치로 조정
        mask_loc = eyes_loc[eyes_loc[:, 1] < 500] - [[50, 40, 0, 0]]

        # Mask 생성
        # 이미지 파일과 동일한 디렉토리
        mask = Image.open(
            '../image_2_style_gan/source/mask_origin/mask_circle_blur.jpg')
        resized_mask = mask.resize(
            (mask_loc[0, 2] * 2, mask_loc[0, 2] * 2))        # 인식된 눈 사이즈 * 2

        # 검정색 1024 x 1024 배경 생성
        back = Image.new("L", (1024, 1024))
        for index in range(len(mask_loc)):
            # 눈이 하나만 인식되면 중앙을 기준으로 y축 대칭 생성
            if len(mask_loc) == 1:
                back.paste(im=resized_mask, box=(
                    mask_loc[index, 0], mask_loc[index, 1]))
                back.paste(im=resized_mask, box=(
                    1024 - mask_loc[index, 0] - mask_loc[index, 2] * 2, mask_loc[index, 1]))
            else:
                back.paste(im=resized_mask, box=(
                    mask_loc[index, 0], mask_loc[index, 1]))
