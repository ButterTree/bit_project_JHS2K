...
def align_images(RAW_IMAGE_DIR, ALIGNED_IMAGE_DIR):
    ...
    for img_name in os.listdir(RAW_IMAGE_DIR):
        ...
        for i, face_landmarks in enumerate(landmarks_detector.get_landmarks(raw_img_path), start=1):
            aligned_face_path = '{}{}_a.png'.format(
                ALIGNED_IMAGE_DIR, os.path.splitext(img_name)[0])
            alinged_files.append(face_align(
                raw_img_path, aligned_face_path, face_landmarks))
